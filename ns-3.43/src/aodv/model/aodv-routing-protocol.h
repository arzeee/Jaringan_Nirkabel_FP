...
#include "ns3/energy-source.h"      // <-- PERUBAHAN 1
#include "ns3/wifi-net-device.h"
#include "ns3/wifi-mac-queue.h"   // <-- PERUBAHAN 2

#include <map>

namespace ns3
{
// NS_LOG_COMPONENT_DECLARE("AodvEocwLogic"); // <-- TAMBAHKAN BARIS INI
class WifiMpdu;
...
namespace aodv
{
    // --- TAMBAHAN EOCW ---
    /**
    * \brief Menyimpan metrik untuk satu path RREQ yang diterima
    */
    struct EocwPath
    {
    double pathMinEnergy;
    double pathAvgCongestion;
    uint32_t hopCount;
...
class RoutingProtocol : public Ipv4RoutingProtocol
{
  public:
    ...
    // --- Fungsi Helper EOCW ---
    /**
     * \brief Menghitung skor Sisa Energi (RE) sesuai Persamaan (9).
     * \return Skor antara 0.0 (habis) dan 1.0 (penuh).
     */
    double GetResidualEnergyScore();

    /**
     * \brief Menghitung skor Tingkat Kemacetan (CD) sesuai Persamaan (8).
     * \return Skor antara 0.0 (penuh) dan 1.0 (kosong).
     */
    double GetCongestionDegreeScore();

    /**
     * \brief Menghitung skor Hop Count (RH) sesuai Tabel 2.
     * \param hopCount Jumlah hop.
     * \return Skor antara 0.1 dan 1.0.
     */
  ...
  private:
    ...
    bool m_enableBroadcast;  ///< Indicates whether a a broadcast data packets forwarding 
    bool m_enableFuzzy;
  ...
  private:
    ...
    // --- Variabel EOCW ---
    /**
     * \brief Pointer ke sumber energi node.
     */
    ns3::Ptr<ns3::energy::EnergySource> m_energySource;
        
    /**
     * \brief Menyimpan energi awal node untuk perhitungan skor.
     */
    double m_initialEnergy;
    // --- End of Variabel EOCW ---
    // --- TAMBAHAN EOCW ---
    /// Peta untuk menyimpan RREQ yang masuk: <RREQ ID, Daftar Path>
    std::map<uint32_t, std::vector<EocwPath>> m_eocwPathCache;

    /// Timer untuk RREQ: <RREQ ID, Timer>
    ...
    */
    ...
    /**
    * \brief Fungsi timer EOCW. Memilih rute terbaik dari cache.
    */
